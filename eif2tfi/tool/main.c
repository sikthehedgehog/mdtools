...
int main(int argc, char **argv) {
   ...
   // Try to read FM instrument from EIF file
   Instrument instr;
   errcode = read_eif(infile, &instr);

   // Try to write FM instrument into TFI file
   if (!errcode)
      errcode = write_tfi(outfile, &instr);
   ...
   if (errcode) {
      ...
      switch(errcode) {
         ...
         case ERR_CORRUPT: msg = "input file isn't a valid Echo FM "
            "instrument"; break;
